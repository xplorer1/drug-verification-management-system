...
public interface AuditLogRepository extends JpaRepository<AuditLog, Long> {
    ...
    @Query("SELECT al FROM AuditLog al WHERE al.blockchainTransactionId IS NULL " +
            "ORDER BY al.createdAt LIMIT :limit")
    List<AuditLog> findUnanchoredLogs(int limit);

    Optional<AuditLog> findTopByOrderByIdDesc();

    @Query("SELECT al FROM AuditLog al WHERE al.previousHash = :hash")
    Optional<AuditLog> findByPreviousHash(String hash);
}
